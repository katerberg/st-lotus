...
export default function CardSparkLines({drafts}) {
  ...
  return (
    ...
    >
    <Sparklines data={drafts.sort((a, b) => (a.occurrence < b.occurrence ? -1 : 1)).map(d => (d.pick ? (max - d.pick / 8) : 0))} max={Math.max(...translatedDrafts)} min={Math.min(...translatedDrafts.filter(d => d))} style={{width: '100%'}}>
